...
### Steps
There will be seperate directory for each of the microservices which will be inside the modules directory

### Kafka Installation and deployment in Kubernetes
...
**3. Kafka Installation Troubleshooting**

Sometimes kafka might throw the below error
...
Issuing the below command will make the error go away
```shell
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
```

### Project deployment in Kubernetes

deploy the project's manifest using the below commands

`kubectl apply -f deployment`

#### Initialize the Postgres Database

We need to execute start up scripts to create database and create tables with some values. This process is shown below:
...
### Verifying it Works
Once the project is up and running, you should be able to see deployments and services for all the microservices, front-end application and postgres db in Kubernetes:
`kubectl get pods` and `kubectl get services` - should both return all the services and deployments
...
### Docker Images
To make changes to the application, build your own Docker image and push it to your own DockerHub repository. 

#### Docker commands to build docker images and push to dockerhub

**1. move inside the respective service folder **
``` shell
cd modules/location_grpc
```
**2. Create docker images**
``` shell
docker build -t grpc-api .
```
**3. Create tags for the docker images**
``` shell
docker tag grpc-api bhk567/grpc-api
```
**4. Push the docker into dockerhub**
``` shell
docker push bhk567/grpc-api
```
